		...
		const disposePoolCredits = ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				try {
					...
					/**
					* The pool rows must carry the same selection flag the settings
					* card's account list uses. Without it every row renders as
					* unselected and clicking one has no visible effect.
					*/
