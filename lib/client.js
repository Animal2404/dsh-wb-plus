...
.dsm-wb-side-pool-region{
  padding:0 6px;border-radius:999px;background:rgba(91,140,255,.14);color:var(--wb-accent);
  font-size:10px;font-weight:600;letter-spacing:.04em;
}
/* Per-row provider badge: the pool aggregates CN + AI, so each row says which. */
.dsm-wb-side-acct-region{
  flex:none;align-self:flex-start;padding:0 5px;border-radius:999px;
  background:rgba(91,140,255,.14);color:var(--wb-accent);
  font-size:9.5px;font-weight:600;letter-spacing:.04em;line-height:14px;
}
...
.dsm-wb-side-pool-stat-expiring{
  border-color:rgba(245,158,11,.28);background:rgba(245,158,11,.06);
}
.dsm-wb-side-pool-stat-expiring span,
.dsm-wb-side-pool-stat-expiring strong{color:var(--dsw-alias-state-warning-primary,#f59e0b)}
...
.dsm-wb-side-acct-token{
  color:var(--wb-text-3);font-size:10.5px;line-height:14px;
  overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
}
...
/* The provider switch now lives on the model card, next to the list it changes. */
.dsm-wb-side-model-region{flex:none;margin-left:auto}
