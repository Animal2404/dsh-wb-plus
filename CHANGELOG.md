...
- 修复共享号池里点击账号没有可见选中状态的问题：宿主 `/pool/credits` 现在返回每个账号的 `selected` 标记，面板据此高亮当前账号并立即刷新。
- 修复切换国内/国际模型区域会带动左侧积分变化的问题：积分卡现在跟随共享号池中选中的账号，区域切换只改变模型列表。
- 号池合并后统一为一个视觉焦点：国内/国际各保留自己的运行账号，但共享卡片只高亮当前查看的那一行，单账号签到也针对该行所属区域执行。
- 回归测试补充账号点击选中与跨区域积分稳定断言，并更新旧版单区域断言。

## 1.2.0

- 号池改为跨区域共享：国内版与国际版的账号、积分、调用统计和限流状态合并展示，汇总数字为两边之和。
- 每个账号行新增 `CN` / `AI` 区域标识；账号切换、单账号签到、令牌到期都按该行所属区域处理。
- 国内/国际切换只改变模型列表，不再切换号池内容。
- 一键签到只针对国内版账号（国际版没有签到接口）。

## 1.1.0

- 默认思考强度：支持 `max` 的模型默认档位改为 `max`（`DeepSeek-V4.1-Flash`、`GLM-5.3`、`Kimi-K2.8`），其余模型按各自最高可用档位。
- 号池汇总新增「近 3 天到期」积分。
- 账号行新增访问令牌到期时间与自动续期提示。
- 号池标题新增 `CN` / `AI` 区域标识。
- 国内/国际切换移动到「选择模型」卡片。
- 移除重复的上游 WorkBuddy 设置卡片，WorkBuddy 页签成为唯一管理入口。
